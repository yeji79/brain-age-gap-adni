...
Ridge 모델은 CN train의 실제 나이를 가장 잘 예측하도록 MRI ROI별 계수를 **학습**합니다. 이후 held-out CN test에서 permutation importance를 계산해, 학습이 끝난 모델의 나이 예측에 실제로 유용했던 ROI 정보를 평가했습니다.

따라서 아래 표의 중요도는 모델 가중치를 조절하기 위해 사전에 부여한 값이 아닙니다. **학습된 모델이 정상군 실제 나이 예측에 활용한 MRI 정보의 사후 평가 결과**입니다. `MAE 증가`는 한 ROI 값을 무작위로 섞었을 때 독립 CN test의 나이 예측 오차가 증가한 크기이며, 클수록 해당 ROI 정보가 예측에 더 유용했음을 뜻합니다.

| MRI ROI | 측정 요소 | 정상 노화에서 함께 관찰될 수 있는 해부학적 방향 | 더 고령의 정상 뇌 구조 방향으로 읽는 예 | MAE 증가(년) |
|---|---|---|---|---:|
...
일반적으로 피질 두께·해마·편도체·일부 회백질 부피가 작고, 뇌실 부피와 백질 고신호량이 큰 MRI 패턴은 더 고령의 정상 뇌 구조 방향과 함께 해석할 수 있습니다. 다만 이는 여러 ROI를 함께 사용하는 모델의 해부학적 맥락이며, 한 ROI의 값만으로 개인의 뇌 나이를 판단할 수는 없습니다.

Ridge 계수의 양수·음수는 표준화된 모든 ROI와 성별을 동시에 고려한 조건부 예측 계수입니다. ROI들 사이에는 높은 상관이 있으므로, 계수 하나만으로 특정 영역이 ‘가장 중요하다’거나 질병의 원인이라고 결론 내리지 않습니다. 이 프로젝트는 held-out CN test permutation importance를 **정상군 실제 나이 예측 기여도**로, 해부학적 방향을 **정상 노화와 함께 읽는 보조 해석**으로 제시합니다.
